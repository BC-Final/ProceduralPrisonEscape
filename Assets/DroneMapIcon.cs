...
public class DroneMapIcon : AbstractMapIcon {
	private void ProcessPacket (NetworkPacket.Update.Drone pPacket) {

	}

	private static void createInstance (NetworkPacket.Update.Drone pPacket) {
		GameObject go = (GameObject)Instantiate(HackerReferenceManager.Instance.DroneIcon, new Vector3(pPacket.PosX / MinimapManager.scale, pPacket.PosY / MinimapManager.scale, 0), Quaternion.Euler(0, pPacket.Rot, 0));

		go.GetComponent<DroneMapIcon>()._health = pPacket.Health;
	}
